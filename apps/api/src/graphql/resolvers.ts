  ...
  Query: {
    ...
    myStats: async (_: unknown, __: unknown, context: Context) => {
      ...
      return {
        ...
        // Round stats to integers as required by GraphQL schema
        strength: Math.round(Number(stats.strength)),
        endurance: Math.round(Number(stats.endurance)),
        agility: Math.round(Number(stats.dexterity)),
        flexibility: Math.round(Number(stats.constitution)),
        balance: Math.round(Number(stats.power)),
        mentalFocus: Math.round(Number(stats.vitality)),
