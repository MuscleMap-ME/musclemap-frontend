...
const muscleStmt = db.prepare(`
  INSERT OR REPLACE INTO muscles (id, name, anatomical_name, muscle_group, bias_weight)
  VALUES (?, ?, ?, ?, ?)
`);

for (const m of muscles) {
  muscleStmt.run(m.id, m.name, m.anatomicalName, m.muscleGroup, m.biasWeight);
}
...
const exerciseStmt = db.prepare(`
  INSERT OR REPLACE INTO exercises (id, name, type, difficulty, primary_muscles)
  VALUES (?, ?, ?, ?, ?)
`);

for (const e of exercises) {
  exerciseStmt.run(e.id, e.name, e.type, e.difficulty, e.primaryMuscles);
}
...
const activationStmt = db.prepare(`
  INSERT OR REPLACE INTO exercise_activations (exercise_id, muscle_id, activation)
  VALUES (?, ?, ?)
`);

for (const a of activations) {
  activationStmt.run(a.exerciseId, a.muscleId, a.activation);
}
