...
${report.fixes.map((f) => `- ${f.bugId}: ${f.success ? '✅' : '❌'} (${f.duration}ms, ${f.filesChanged} files)`).join('\n')}
...
`;
  ...
  logBugFound(bug: DiagnosedBug): void {
    ...
    console.log(`${color}❌ [${bug.severity.toUpperCase()}]${reset} ${bug.title}`);
    console.log(`   └─ ${bug.rootCause.hypothesis}`);
    console.log(`   └─ File: ${bug.rootCause.file}`);
  }
